...
st.set_page_config(page_title="Simulação de Monte Carlo", layout="wide")
...
dados_filtrados = dados[str(data_ini) <= dados["Data"]]
dados_filtrados = dados[dados["Data"] <= str(data_fim)]
...
with st.expander("Formulário de simulação", expanded=True):
    meta = st.number_input(label="Meta", value=10000, placeholder="Meta")
    ...
    if submit:
        ...
        media_sim, mediana_sim, desvio_sim, p80_inf_sim, p80_sup_sim, prob_meta_sim, resultados_sim = simular_monte_carlo(gerar_dados_amostrais(st.session_state.melhor_distribuicao, st.session_state.melhor_parametro), meta, n_sim, n_dias)
        ...
        plt.axvline(x=p80_inf_sim, color='blue', linestyle='--', label=f'{p80_inf_sim:.2f} (P80 Inferior)')
        plt.axvline(x=p80_sup_sim, color='blue', linestyle='--', label=f'{p80_sup_sim:.2f} (P80 Superior)')
        plt.xlabel('Produção Total')
        plt.ylabel('Densidade de Probabilidade')
        plt.title(f'''Distribuição da Produção Total\nDado: {nome_dado}\nDistribuição: {definir_nome_distribuicao(st.session_state.melhor_distribuicao)}''')
        plt.legend()
        ...
        with col6:
            ...
            with st.container(border=True):
                st.metric("P80 Inferior", f"{p80_inf_sim:.2f}")
        with col7:
            with st.container(border=True):
                st.metric("Mediana", f"{mediana_sim:.2f}")
            with st.container(border=True):
                st.metric("P80 Superior", f"{p80_sup_sim:.2f}")
        with col8:
            with st.container(border=True):
